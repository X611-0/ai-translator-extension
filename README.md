...
> 赛题二：AI 同声传译助手 — Chrome 浏览器扩展

实时将外语视频/音频流翻译为中文，以**字幕 + 语音**双模式呈现，帮助用户跨越语言障碍，高效获取信息。

## 快速体验

1. 克隆项目，在 `chrome://extensions/` 中加载 `dist/` 目录
2. 打开扩展**选项页面**，填入讯飞和阿里云的 API 密钥（或使用已内置的测试密钥）
3. 打开任意外语视频网页，点击扩展图标 → **开始翻译**
4. 页面底部出现半透明字幕浮层，实时显示原文和译文
...
### 讯飞语音识别（必须）
1. 注册 [讯飞开放平台](https://console.xfyun.cn/)，创建应用
2. 开通 **实时语音转写 (AST)** 服务
3. 在扩展选项页面填入 App ID、API Key、API Secret

### 阿里云机器翻译（必须）
1. 登录 [阿里云 RAM 访问控制](https://ram.console.aliyun.com/)，获取 AccessKey
2. 开通 [机器翻译服务](https://mt.console.aliyun.com/)
3. 在扩展选项页面填入 AccessKey ID 和 Secret

### 语音播报（可选）
1. 在 [智能语音交互控制台](https://nls.console.aliyun.com/) 创建项目，获取 AppKey
2. 在扩展选项页面填入语音合成 AppKey
